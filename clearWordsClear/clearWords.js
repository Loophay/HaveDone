window.onload=function(){
  ...
  var lev=document.getElementsByTagName('span')[2];
  // 记录 生命 分数
  var scoreNum=0;
  var hpNum= 10;
  var level=1; 
  ...
  function getChar(){
        ...
        */
        ...
        var div=document.createElement('div');
         // 位置
        var left=(cw-400)*Math.random()+200;
        while(checkPosition(left,positionArr)){
              left=(cw-400)*Math.random()+200;
           }
  ...
  function checkPosition(left,po){
        for(var i=0;i<po.length;i++){
         if(!(left + 100 < po[i] ||  po[i]+100<left )){
             return true;
         }
      }
      return false;
       ...
       for(var i=0;i<currentArr.length;i++){
          ...
          // currentArr[i].offsetTop
           ...
           if(v>500){
              ...
              if(hpNum<=0){
                clearInterval(t);
                var start=confirm('游戏结束！本次得分：'+scoreNum+'是否继续？');
                if(start){
                  restart();
                  return;
                }else{
